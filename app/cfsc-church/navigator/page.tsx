  ...
  const fetchCfscFiles = async () => {
    try {
      ...
      if (data.success && data.records?.length) {
        ...
        const displayName = (f: CfscChurchRecord) => (!f.fileName.toLowerCase().endsWith('.pdf') && f.sermonTitle) ? f.sermonTitle : f.fileName;
      ...
      {sermonContent ? (
        <div className={styles.contentWrapper}>
          ...
          <div className={styles.chatSection}>
            ...
            {user && (
              <ChatkitEmbed userId={user.user_id} module="cfsc-church-navigator" />
            )}
          </div>
        </div>
      ) : (
