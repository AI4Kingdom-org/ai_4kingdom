/**
 * 講章內容生成核心（summary / devotional / bibleStudy）。
 *
 * 不依賴 Next.js，兩處共用：
 * - Fly.io worker（生產環境）：services/youtube-audio-worker 以 esbuild 打包本檔後在背景執行，
 *   不受 Amplify 28 秒請求上限影響。
 * - app/api/sunday-guide/process-document（本機開發、或 worker 無法接手時的後備）。
 *
 * 修改本檔後需重新打包並部署 worker：cd services/youtube-audio-worker && npm run deploy
 */
...
// 由 runProcessDocument 依呼叫端選項建立（worker 可注入走 proxy 的 fetch）
let openai: OpenAI;
...
export interface ProcessDocumentOptions {
  /** 自訂 fetch（例如 worker 透過 OPENAI_HTTP_PROXY 連線） */
  fetch?: (url: any, init?: any) => Promise<any>;
}

// 分頁掃描取得該 fileId 最新的一筆記錄。
// optimizedQuery 只掃第一頁，表變大後會漏抓，導致 processing / failed 狀態寫不回原記錄。
async function findLatestRecord(docClient: any, fileId: string, unitId?: string) {
  const items: any[] = [];
  let lastKey: any = undefined;
  do {
    const res = await docClient.send(new ScanCommand({
      TableName: SUNDAY_GUIDE_TABLE,
      FilterExpression: unitId ? 'fileId = :fid AND unitId = :unitId' : 'fileId = :fid',
      ExpressionAttributeValues: unitId ? { ':fid': fileId, ':unitId': unitId } : { ':fid': fileId },
...
/**
 * 將該講章標記為 failed，讓前端輪詢（check-result）立即看到錯誤而不是等到逾時。
 * 找不到原記錄時建立一筆 failed 記錄。
 */
export async function recordProcessingFailure(req: ProcessDocumentRequest, errorMsg: string) {
  const { assistantId, vectorStoreId, fileName, fileId, userId, unitId } = req;
  const docClient = await createDynamoDBClient();
  const latest = await findLatestRecord(docClient, fileId || vectorStoreId);
  if (latest) {
...
// 請求未帶 fileId 時，從 DynamoDB 記錄或向量庫推斷
async function resolveFileId(req: ProcessDocumentRequest): Promise<string | undefined> {
  const { vectorStoreId, fileName, unitId } = req;
  if (req.fileId) {
    console.log('[DEBUG] 從請求取得 fileId:', req.fileId);
    return req.fileId;
  }
  try {
    ...
    if (result.Items && result.Items.length > 0) {
      ...
      if (latestItem.fileId) {
        console.log(`[DEBUG] 從數據庫找到文件 ID: ${latestItem.fileId}`);
        return latestItem.fileId;
      }
    }
  } catch (dbError) {
  ...
  try {
    ...
    if (filesInVectorStore.data.length >= 1) {
      ...
      console.log(`[DEBUG] 從向量庫取最新加入的檔案 ID: ${sortedFiles[0].id}（共 ${filesInVectorStore.data.length} 個檔案）`);
...
/** 生成講章內容並寫回 DynamoDB。成功回傳 true；失敗時已寫入 failed 狀態並回傳 false。 */
export async function runProcessDocument(req: ProcessDocumentRequest, options: ProcessDocumentOptions = {}) {
  openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY, ...(options.fetch ? { fetch: options.fetch as any } : {}) });
  const fileId = await resolveFileId(req);
  return processDocumentAsync({ ...req, fileId });
}
...
async function processDocumentAsync(params: {
  ...
  try {
    const latest = await findLatestRecord(docClient, fileId || vectorStoreId, unitId);
    if (latest) {
      await docClient.send(new UpdateCommand({
        TableName: SUNDAY_GUIDE_TABLE,
        Key: { assistantId: latest.assistantId, Timestamp: latest.Timestamp },
...
- If uncertain about content, write "[MISSING]" rather than guessing.`
  ...
  // docClient 已於函數頂部建立
    ...
    try {
      ...
      await recordProcessingFailure({ assistantId, vectorStoreId, fileName, fileId, userId: effectiveUserId, unitId }, errorMsg);
