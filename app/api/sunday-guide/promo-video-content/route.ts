...
export async function GET(request: Request) {
  try {
    ...
    if (!upstream.ok) {
      ...
      return NextResponse.json(
        {
          error: 'PROMO_VIDEO_CONTENT_FAILED',
          message: data?.error?.message || data?.message || `讀取影片內容失敗 (${upstream.status})`,
        },
    ...
    return NextResponse.json(
      {
        error: 'PROMO_VIDEO_CONTENT_EXCEPTION',
        message: error instanceof Error ? error.message : '讀取影片內容失敗',
      },
      { status: 500 },
    );
  }
