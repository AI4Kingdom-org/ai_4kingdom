...
- **模型与指令**：不再有平台侧的 Assistant 对象。每次调用需指定 model/instructions，由 `app/lib/openai/profiles.ts` 解析：优先读取 `app/config/assistantProfiles.ts` 快照，其次实时抓取旧 assistant 配置（日落前有效），兜底用 `OPENAI_RESPONSES_MODEL` 环境变量（默认 `gpt-4o`）。
